...
// Ensures that server request method must be POST to run the script
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $productName = $_POST['product_name'] ?? '';
    $description = $_POST['description'] ?? '';
    $category_id = intval($_POST['category_id']);
    $price = floatval($_POST['price']); // gets the post request value and gets the Float value of the user (admin) input
    $pricePerDay = floatval($_POST['price_per_day']);
    $availableQty = intval($_POST['available_qty']); // gets the post request value and gets the integer value of the user (admin) input
    $imagePath = $_POST['product_image'] ?? ''; // gets the image url path inputed by the admin which will correlate to the image file in the application folder for a particular image

    // Checks that most fields are filled up before running the insert SQL query to the products table
    if ($productName && $description && $price && $pricePerDay && $availableQty) {
...
<body>
    ...
    <form action="" method="post">
        ...
        <input type="number" name="category_id"  required></input><br><br>
